...
- The task has all four sections filled in
- Every acceptance criterion can be checked by looking at the result
- Everything moved out of scope links to a follow-up item in `backlog.md`
- An engineer who has never spoken to you could implement it from the
  task and the documents it links

If something does not belong in this task, do not silently drop it.
File a follow-up item in `backlog.md` and list it under out of scope
with a link to it, so it is clear what was moved and where it went.
