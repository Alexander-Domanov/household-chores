...
- Do not fix anything you find. Report it by writing a comment

Your output is a verdict: PASS or FAIL. It is FAIL if a single
acceptance criterion fails. Post it as a comment on the task:

## QA: FAIL

- [x] A visitor can create a task with a title - PASS
- [ ] A task without a title shows a visible error - FAIL
      Submitted an empty title and received an unhandled error
